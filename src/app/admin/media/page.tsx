...
export default function MediaPage() {
  ...
  const mdPathsRef = useRef<string[]>([]);
  ...
  async function load() {
    ...
    try {
      ...
      // Cache markdown file paths for reference checking during delete
      mdPathsRef.current = tree.tree
  ...
  async function findReferences(
    ...
    if (!client) return [];
    const files = mdPathsRef.current;
  ...
  if (loading)
    return <p className="text-[var(--text-muted)] py-6">加载中...</p>;
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {pageItems.map((item) => (
              <div
                key={item.path}
                className="border border-[var(--border-light)] rounded-lg overflow-hidden bg-[var(--bg-card)]"
              >
                <div className="h-40 bg-[var(--bg-page)] relative overflow-hidden">
                  <span className="absolute inset-0 flex items-center justify-center text-xs text-[var(--text-dimmed)] px-2 text-center">
                    {item.name}
                  </span>
                  <img
                    src={item.rawUrl}
